...
def test_fill_market_price():
    ...
    assert data_filled_market[(data_filled_market["player"]=="carlos-dominguez") & 
                       (data_filled_market["season"]==2026) & 
...
def test_add_team_strength_feature():
    ...
    assert data_teams[(data_teams["player"]=="tchouameni") & 
                       (data_teams["season"]==2026)]["player_team_strength"].unique().size == 1
    
    assert data_teams[(data_teams["player"]=="tchouameni") & 
                       (data_teams["season"]==2026)]["player_team_strength"].unique().item(0) == 658
...
def test_price_change_time_ratio():
    ...
    assert data_price_change_ratio[(data_price_change_ratio["player"]=="alvaro-garcia") & 
                       (data_price_change_ratio["season"]==2026) & 
                       (data_price_change_ratio["fixed_round"]==4)]["price_change_time_ratio"].iloc[0] >= -0.39
    
    assert data_price_change_ratio[(data_price_change_ratio["player"]=="alvaro-garcia") & 
                       (data_price_change_ratio["season"]==2026) & 
                       (data_price_change_ratio["fixed_round"]==4)]["price_change_time_ratio"].iloc[0] <= -0.37
...
def test_past_rolling_avgs():
    ...
    assert data_rolling_past[(data_rolling_past["player"]=="alvaro-garcia") & 
                       (data_rolling_past["season"]==2026) & 
                       (data_rolling_past["fixed_round"]==4)]["puntuacion_media_roll_avg_3"].iloc[0] == 19/3
...
def test_calculate_injury_severity():
    ...
    assert data_injury_severity[(data_injury_severity["player"]=="alvaro-garcia") & 
                       (data_injury_severity["season"]==2026) & 
